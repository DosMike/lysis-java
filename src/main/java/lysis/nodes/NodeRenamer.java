	...
	private void renameBlock(NodeBlock block) throws Exception {
		for (NodeList.iterator iter = block.nodes().begin(); iter.more();) {
			...
			if (node.uses().size() == 2) {
				...
				if (firstUse.node().type() == NodeType.Store && (secondUse.node().type() == NodeType.Binary
				...
				// new var1 = a;
				// if(var1 > 10 & 0 < var1)
				if (firstUse.node().type() == NodeType.Binary && secondUse.node().type() == NodeType.Binary) {
					// See if they're both connected through a binary |and| expression
					if (firstUse.node().uses().size() == 1 && secondUse.node().uses().size() == 1
